...
const capturedWorkerEventHandlers: Map<
  string,
  Map<string, Function>
> = new Map();
...
describe("MLFQScheduler - Specification Tests", () => {
  ...
  describe("SPEC-SCHED-003: MLFQ Rules", () => {
    ...
    it("should stop BoostManager on shutdown", async () => {
      await scheduler.initialize();
      const boostManagerInstance = (BoostManager as jest.Mock).mock.results[0]
        .value;
      await scheduler.shutdown();
  ...
  describe("submit() - Job Submission", () => {
    ...
    it("should handle multiple submissions", async () => {
      ...
      expect((scheduler as any).jobMetadata.get(request1.id).queueLevel).toBe(
        0,
      );
      expect((scheduler as any).jobMetadata.get(request2.id).queueLevel).toBe(
        0,
      );
    });
  });
  ...
  describe("boostAllJobs() - Rule 5", () => {
    ...
    it("should boost jobs from lower queues", async () => {
      ...
      const queues = (scheduler as any).queues;
      await queues[1].add("llm-request-" + request.id, metadata, {
        jobId: request.id,
      });
  ...
  describe("Worker Event Handlers", () => {
    ...
    it("should handle completed event", async () => {
      ...
      const completedHandler = capturedWorkerEventHandlers
        .get("mlfq-q0")
        ?.get("completed");
      if (completedHandler) {
    ...
    it("should handle failed event", async () => {
      ...
      const failedHandler = capturedWorkerEventHandlers
        .get("mlfq-q0")
        ?.get("failed");
      if (failedHandler) {
        failedHandler(
          { id: "job-1", data: { requestId: request.id } },
          new Error("Test"),
        );
        expect((scheduler as any).jobMetadata.has(request.id)).toBe(false);
      }
  ...
  describe("MongoDB Logging", () => {
    ...
    it("should handle logging errors gracefully", async () => {
      ...
      await expect(
        logRequest(request, RequestStatus.QUEUED, new Date(), 0),
    ...
    it("should update queue level", async () => {
      ...
      const updateQueueLevel = (scheduler as any).updateQueueLevel.bind(
        scheduler,
      );
    ...
    it("should handle updateQueueLevel errors", async () => {
      ...
      await expect(
        updateQueueLevel("test-id", 1, [0, 1]),
      ).resolves.not.toThrow();
    });
  });
  ...
  describe("Integration Tests", () => {
    ...
    it("should handle multiple concurrent jobs", async () => {
      ...
      const jobIds = await Promise.all(
        requests.map((r) => scheduler.submit(r)),
      );
      ...
      const results = await Promise.all(
        requests.map((r) => scheduler.cancel(r.id)),
      );
