...
describe("WFQScheduler - Business Logic Tests", () => {
  ...
  describe("DEFAULT_WEIGHTS - Tenant Weight Constants", () => {
    ...
    it("should have monotonically decreasing weights", () => {
      expect(DEFAULT_WEIGHTS[TenantTier.ENTERPRISE]).toBeGreaterThan(
        DEFAULT_WEIGHTS[TenantTier.PREMIUM],
      );
      expect(DEFAULT_WEIGHTS[TenantTier.PREMIUM]).toBeGreaterThan(
        DEFAULT_WEIGHTS[TenantTier.STANDARD],
      );
      expect(DEFAULT_WEIGHTS[TenantTier.STANDARD]).toBeGreaterThan(
    ...
    it("should have weight ratio of 2x between consecutive tiers", () => {
      ...
      expect(
        DEFAULT_WEIGHTS[TenantTier.STANDARD] / DEFAULT_WEIGHTS[TenantTier.FREE],
      ).toBe(10);
    });
  });
  ...
  describe("updateTenantWeight - Weight Updates", () => {
    ...
    it("should update weight to valid values", () => {
      ...
      weights.forEach((weight) => {
        const result = scheduler.updateTenantWeight(
          "weight-update-test",
          weight,
        );
        if (result) {
  ...
  describe("Virtual Time Calculation", () => {
    ...
    it("should give higher weight tenants lower virtual finish time", () => {
      ...
      expect(enterpriseFinish.virtualFinishTime).toBeLessThan(
        freeFinish.virtualFinishTime,
      );
    });
