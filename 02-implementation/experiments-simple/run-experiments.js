...
function analyzeResults(results, schedulerName) {
  ...
  for (const p of [PRIORITY.LOW, PRIORITY.NORMAL, PRIORITY.HIGH, PRIORITY.URGENT]) {
    const pResults = results.filter(r => r.priority === p);
    if (pResults.length > 0) {
      priorityWaitTimes[p] = pResults.reduce((sum, r) => sum + r.waitTime, 0) / pResults.length;
    }
  }
  ...
  for (const tenant of TENANTS) {
    ...
    if (tResults.length > 0) {
      tenantWaitTimes[tenant.id] = tResults.reduce((sum, r) => sum + r.waitTime, 0) / tResults.length;
    }
  }
...
function main() {
  ...
  console.log(`  - 처리 시간 범위: ${CONFIG.processingTimeMin}-${CONFIG.processingTimeMax}ms`);
  console.log();
  ...
  console.log('\n[1] 기본 성능 지표\n');
  console.log('스케줄러      | 평균대기(ms) | 최대대기(ms) | 처리량(req/s) | 공정성');
  ...
  for (const s of allStats) {
    ...
    console.log(`  ${s.scheduler}: ${sign}${improvement}% (${s.avgWaitTime}ms vs ${fcfsStats.avgWaitTime}ms)`);
  }
  ...
  for (const [tenant, wait] of Object.entries(wfqStats.tenantWaitTimes)) {
    ...
    console.log(`  ${tenant.padEnd(12)} (weight: ${String(tenantInfo.weight).padStart(3)}): ${wait.toFixed(2)}ms`);
  }
