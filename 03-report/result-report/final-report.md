...
- **로컬 LLM 사용**: 비용 제약으로 Ollama 로컬 LLM을 사용하였으며, 실제 OpenAI/Claude API의 네트워크 지연 및 Rate Limiting 동작과 차이가 있을 수 있음.
- **단일 머신 환경**: 분산 환경이 아닌 단일 머신에서 실험하였으며, 실제 프로덕션 환경의 네트워크 지연, 부하 분산 등의 요소가 반영되지 않음.
- **워크로드 다양성**: 합성 워크로드(Synthetic Workload)를 사용하였으며, 실제 사용자 요청 패턴과 차이가 있을 수 있음.

향후 연구에서는 실제 OpenAI/Claude API를 활용한 검증과 분산 환경에서의 성능 분석을 수행할 예정이다.
