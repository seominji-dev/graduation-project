...
실험 결과, MLFQ가 평균 대기 시간을 FCFS 대비 40% 개선하였고, 처리량은 20% 증가하였다. WFQ는 개별 테넌트 수준에서 0.92-0.98의 Jain's Fairness Index를 달성하여 높은 공정성을 보여주었다. 전체 시스템은 777개 테스트에 100% 통과하였고, 코드 커버리지 98.72%(Statements)를 달성하였다. 본 연구는 OS 이론의 AI 시스템 응용 가능성을 실증하며, 실무적으로도 SaaS 멀티테넌트 서비스, 고객 지원 시스템, 대화형 AI 플랫폼 등에 활용될 수 있는 실용적인 솔루션을 제공한다.
...
4. **품질 보증**: 85% 이상 테스트 커버리지 달성, 코드 품질 관리
...
**기존 연구와의 차별점 (Differentiation from Prior Work):**

본 연구는 기존 API 관리 솔루션 및 OS 스케줄링 연구와 다음과 같은 명확한 차별점을 가진다:

| 비교 항목 | 기존 솔루션 | 본 연구 |
|----------|------------|---------|
| **Rate Limiting** | OpenAI/Anthropic의 Token Bucket 기반 단순 제한 | 우선순위, 공정성, 적응형 스케줄링 통합 |
| **Task Queue** | Celery/BullMQ의 단일 FIFO 큐 | 4가지 알고리즘 런타임 교체, MLFQ 다단계 큐 |
| **Time Quantum** | OS 기본값 (10-100ms) | LLM 응답 패턴 기반 보정 (1,000ms~8,000ms) |
| **Starvation 방지** | 일반적 timeout 기반 | Aging(30초)/Boosting(60초) 메커니즘 |
| **Fairness 측정** | 미제공 | Jain's Fairness Index 실시간 추적 |
| **멀티테넌트** | 테넌트별 Rate Limit만 제공 | WFQ 기반 가중치 공정 분배 |

**핵심 기술적 차별점:**

1. **LLM 특화 Time Quantum 보정**: 일반 OS 스케줄러(10-100ms)와 달리, 실제 LLM 응답 시간 분포(평균 2-5초)를 분석하여 1,000ms/3,000ms/8,000ms 타임 퀀텀을 설계. 이는 네트워크 지연과 토큰 생성 시간을 고려한 최적화임.

2. **API 인식 Starvation 방지**: CPU 바운드 프로세스가 아닌 I/O 바운드 API 요청의 특성을 반영하여, Aging 임계값을 30초(일반 OS의 수 ms 대비 1000배)로 설정. 이는 API 호출의 높은 지연 분산을 수용함.

3. **SaaS 멀티테넌트 공정성**: 네트워크 패킷 스케줄링의 WFQ를 SaaS 계층형 가격 모델(Free/Pro/Enterprise)에 적용하여, 비용 지불 수준에 비례한 공정한 API 할당 구현. 기존 연구에서 다루지 않은 새로운 적용 영역임.

4. **통합 스케줄링 프레임워크**: FCFS/Priority/MLFQ/WFQ를 동일한 인터페이스(`IScheduler`)로 통합하여 런타임 알고리즘 교체 지원. 워크로드 변화에 따른 적응형 스케줄링 전략 선택 가능.
...
- ESLint, Prettier, TypeScript 정적 타이핑으로 코드 품질 관리
...
본 연구의 실험은 다음과 같은 체계적인 방법론을 따랐다:

1. **실험 설계**: 각 스케줄링 알고리즘에 대해 동일한 워크로드(100개 요청/구성)를 적용하여 통제된 비교 실험 수행. 각 실험은 3회 반복 측정하여 평균값 산출.

2. **측정 지표**: 대기 시간(Waiting Time), 처리 시간(Turnaround Time), 처리량(Throughput), P95 지연(95th Percentile Latency), Jain's Fairness Index를 측정.

3. **통계적 검증**: 측정값의 표준편차는 평균 대비 15% 이내로 확인. 알고리즘 간 성능 차이는 t-검정(유의수준 α=0.05) 결과 통계적으로 유의미함(p < 0.01).

4. **재현성 보장**: 모든 실험은 Docker Compose 환경에서 수행되었으며, 동일한 랜덤 시드(seed=42)를 사용하여 재현 가능.

**Ollama 선택 이유 (Rationale for Ollama):**

본 연구에서 Ollama(로컬 LLM)를 주요 실험 환경으로 선택한 것은 학술적으로 타당한 결정이다:

- **재현성 (Reproducibility)**: 학술 연구의 핵심 요소로, 로컬 환경에서 동일 조건으로 반복 실험 가능
- **통제된 환경**: 네트워크 변동 없이 순수 알고리즘 성능만 측정 가능
- **비용 효율성**: 777개 테스트를 수백 번 반복 실행해도 비용 발생 없음
- **학술 표준**: CS 연구에서는 통제된 환경 실험 후 실제 환경 검증을 순차적으로 진행하는 것이 일반적

**실험적 한계 (Experimental Limitations):**

- **로컬 LLM 사용**: 실제 OpenAI/Claude API의 네트워크 지연(100-500ms), Rate Limiting, 시간대별 응답 변동성을 완전히 재현하지 못함
- **단일 머신 환경**: 분산 환경의 네트워크 지연, 부하 분산, 노드 간 동기화 등의 요소 미반영
- **워크로드 다양성**: 합성 워크로드(Synthetic Workload) 사용으로 실제 사용자 요청 패턴과 차이 존재

**학술적 의의:**

본 연구의 가치는 통제된 환경에서 OS 스케줄링 알고리즘의 LLM 도메인 적용 가능성을 실증한 것에 있음:
- **내적 타당성 (Internal Validity)**: Ollama 로컬 환경은 외부 변수(네트워크, Rate Limiting)를 제거하여 순수 알고리즘 성능 비교 가능
- **재현성 (Reproducibility)**: 777개 테스트 케이스로 실험 결과 재현 가능
- **후속 연구 기반**: Strategy Pattern 기반 설계로 다양한 LLM 제공자 환경에서의 확장 연구 용이
...
| 가독성 | 명확한 네이밍, TypeScript 타입 시스템 |
| 일관성 | ESLint, Prettier 코드 스타일 적용 |
| 안전성 | Zod 입력 검증, 에러 처리 |
| 추적성 | 구조화된 로깅, MongoDB 저장 |
...
- 전체 시스템: 777개 테스트 100% 통과, 커버리지 98.72%
...
**실무적 가치:**
- 높은 테스트 커버리지(98.72%), 일관된 코드 품질 관리
...
**논문 제출일:** 2026년 1월 30일 (최종 수정: 2026년 2월 1일)
**완성 보고서 작성자:** 홍익대학교 컴퓨터공학과 C235180 서민지
**프로젝트 기간:** 2026년 3월 ~ 2026년 2월
**총 코드 라인 수:** 6,180 lines (TypeScript)
**총 테스트 수:** 777 tests (100% pass)
**코드 커버리지:** 98.72% (Statements), 85.77% (Branches)

---

*본 논문은 홍익대학교 2026년 졸업 프로젝트로 제출되었으며, MIT 라이선스 하에 오픈 소스로 공개됨.*
