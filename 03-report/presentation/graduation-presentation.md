...
**기존 연구와의 차별점:**
- 기존 LLM 연구: GPU 메모리 최적화, 추론 속도 향상 ("더 빠르게")
- 본 연구: OS 스케줄링 이론을 LLM에 적용 ("더 공정하게")

**3가지 핵심 기여:**

1. **정량적 공정성 측정 방법론**
   - Jain's Fairness Index로 공정성을 0~1 지표로 정량화
   - 시스템 수준(전체 분배) + 테넌트 수준(개별 공정성) 이중 측정

2. **OS 이론의 LLM 도메인 체계적 적용**
   - FCFS, Priority, MLFQ, WFQ 4가지 알고리즘 비교
   - 네트워크 WFQ 이론과 OS 기아 방지 메커니즘 융합

3. **검증 가능한 재현성 제공**
   - 69개 테스트 100% 통과, 98.65% 커버리지
   - Priority: URGENT 요청 62.3% 개선 (1,122ms vs 2,971ms)
   - WFQ: Enterprise가 Free보다 5.8배 빠른 응답 (849ms vs 4,894ms)

### 실용적 기여

- ✓ 런타임 알고리즘 교체 가능 구조 (Strategy Pattern)
...
**결과:** FCFS와 MLFQ의 대기 시간이 동일함 (63,107ms, 0% 개선율)

### 한계점 및 원인 분석

**문제:** MLFQ의 큐 이동 메커니즘이 작동하지 않음

**원인:** 현재 시뮬레이션은 **선점형(Non-preemptive)** 방식으로 작동
- 모든 요청이 먼저 큐에 삽입됨
- 요청이 하나씩 순차적으로 처리됨 (완전히 완료된 후 다음 요청)
- 각 요청이 처리 시간을 모두 소진 후 `feedback()`이 호출되지만, 이미 처리가 완료되었으므로 큐 이동이 의미 없음

### 향후 연구 방향

**시간 분할(Time-sliced) 시뮬레이션 필요:**
- 타임 퀀텀(500ms) 단위로 처리
- 타임 퀀텀 경과 시 요청이 완료되지 않았으면 큐 강등 (Q0 → Q1 → Q2)
- 다음 요청 처리 시 상위 큐의 요청 우선 처리
...
**1. 연구 관점의 전환: "속도" → "공정"**
- 기존 LLM 연구는 GPU 메모리 최적화, 추론 속도에 집중
- 본 연구는 OS 스케줄링 이론으로 **공정성과 기아 방지**에 초점

**2. 정량적 공정성 측정 방법론 제시**
- Jain's Fairness Index로 공정성을 0~1 지표로 정량화
- 시스템 수준(0.32) + 테넌트 수준(0.92-0.98) 이중 측정
- Provider가 실시간으로 공정성 모니터링 가능

**3. 엄격한 실증적 검증**
- 69개 테스트 100% 통과, 98.65% 커버리지
- Priority: URGENT 요청 62.3% 개선 실험적 입증
- WFQ: Enterprise 5.8배 빠른 응답 검증

**4. 교육적 가치**
- OS 이론과 실제 구현의 간극 해소
- 7개 학습 자료(3,799줄)로 교육용 레퍼런스 제공
...
![코드 커버리지](../graphs/code-coverage.png)
