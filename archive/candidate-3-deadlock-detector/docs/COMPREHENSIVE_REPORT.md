...
**프로젝트명:** AI 멀티 에이전트 시스템을 위한 데드락 감지 및 회복 시스템
**SPEC ID:** SPEC-DEAD-001
**버전:** 1.0.0
**작성일:** 2026-01-25
...
1. [프로젝트 개요](#1-프로젝트-개요)
2. [Wait-For Graph 기반 탐지](#2-wait-for-graph-기반-탐지)
3. [5가지 희생자 선택 전략](#3-5가지-희생자-선택-전략)
4. [은행원 알고리즘](#4-은행원-알고리즘)
5. [체크포인트 및 롤백](#5-체크포인트-및-롤백)
6. [테스트 결과](#6-테스트-결과)
...
현대 AI 시스템에서 다중 에이전트 아키텍처가 급속도로 확산되고 있습니다. ChatGPT, Claude, Gemini 등의 대형 언어 모델(LLM)을 기반으로 한 에이전트들이 복잡한 작업을 수행하기 위해 협력하는 환경에서, 공유 자원에 대한 경쟁은 필연적으로 발생합니다.

이러한 환경에서 데드락(교착 상태)은 시스템 전체의 진행을 멈추게 하는 치명적인 문제입니다. 본 프로젝트는 운영체제 이론에서 검증된 데드락 감지 및 회복 기법을 AI 멀티 에이전트 시스템에 적용하여, 자원 경합으로 인한 교착 상태를 자동으로 탐지하고 해결하는 시스템을 구현했습니다.

### 1.2 프로젝트 목표

| 목표 | 설명 | 달성 상태 |
|------|------|-----------|
| Wait-For Graph 구현 | 에이전트 간 대기 관계를 그래프로 모델링 | 완료 |
| DFS 기반 사이클 탐지 | O(V+E) 시간 복잡도의 효율적인 탐지 알고리즘 | 완료 |
...
Wait-For Graph(WFG)는 에이전트 간의 자원 대기 관계를 표현하는 방향 그래프입니다. 그래프에서 사이클이 존재하면 데드락이 발생했음을 의미합니다.

**그래프 구성 요소:**
- **노드 (Nodes):** 시스템 내의 에이전트
- **엣지 (Edges):** 에이전트 A가 에이전트 B가 보유한 자원을 대기 중임을 나타냄
- **사이클 (Cycle):** 순환 대기 상태, 즉 데드락
...
Depth-First Search(DFS)를 사용하여 그래프에서 사이클을 탐지합니다. 노드 방문 상태를 3가지 색상으로 관리하는 "Three-Color" 알고리즘을 적용했습니다.

**노드 상태:**
- **WHITE (UNVISITED):** 아직 방문하지 않음
- **GRAY (VISITING):** 현재 DFS 재귀 스택에 있음
- **BLACK (VISITED):** DFS 탐색 완료

**사이클 탐지 원리:**
- DFS 탐색 중 GRAY 상태의 노드를 다시 만나면 **백 엣지(Back Edge)**가 존재
- 백 엣지의 존재 = 사이클 존재 = 데드락 발생
...
데드락 해결을 위해 사이클에 포함된 에이전트 중 하나를 종료(희생자)해야 합니다. 어떤 에이전트를 선택하느냐에 따라 시스템 성능과 공정성이 달라집니다.
...
**단점:**
- 낮은 우선순위 에이전트의 기아(Starvation) 현상 가능
...
| 전략 | 시간 복잡도 | 공정성 | 효율성 | 예측가능성 | 권장 환경 |
|------|------------|--------|--------|-----------|----------|
| Lowest Priority | O(n) | 낮음 | 높음 | 높음 | 우선순위 명확한 시스템 |
| Youngest | O(n) | 중간 | 중간 | 높음 | 작업 진행도 중요한 시스템 |
| Most Resources | O(n) | 중간 | 높음 | 높음 | 자원 경합 심한 시스템 |
| Min Dependencies | O(n*m) | 높음 | 높음 | 중간 | 의존성 복잡한 시스템 |
...
은행원 알고리즘(Banker's Algorithm)은 데드락 **회피(Avoidance)** 기법입니다. 자원 할당 전에 시스템이 안전 상태(Safe State)인지 검사하여, 안전하지 않은 요청은 거부합니다.

### 4.2 핵심 개념

**안전 상태 (Safe State):**
- 모든 에이전트가 최대 자원을 요청하더라도 데드락 없이 완료 가능한 상태
- 안전 순서(Safe Sequence)가 존재하면 안전 상태

**안전 순서 (Safe Sequence):**
- 에이전트들이 순서대로 실행하면 모두 완료할 수 있는 순서
- 예: [Agent-1, Agent-3, Agent-2]
...
데드락 해결 시 희생자 에이전트를 종료한 후, 상태를 복구하여 작업을 재개할 수 있도록 체크포인트 기반 롤백 메커니즘을 제공합니다.
...
1. **체크포인트 생성:** 에이전트 상태를 직렬화하여 저장
2. **롤백 요청:** 저장된 체크포인트 조회
3. **상태 복구:** 에이전트 상태를 체크포인트 시점으로 복원
4. **자원 해제:** 체크포인트 이후 획득한 자원 반환

### 5.4 체크포인트 관리

- **최대 체크포인트 수:** 에이전트당 10개 (설정 가능)
- **자동 삭제:** FIFO 방식으로 오래된 체크포인트 제거
- **스토리지:** 인메모리 또는 Redis/MongoDB
...
| **전체** | **66.64%** | **83.07%** | **85.54%** | **66.64%** |

### 6.3 TRUST 5 품질 점수

| 항목 | 점수 | 근거 |
|------|------|------|
| **Tested** | 95/100 | 63/63 테스트 통과, 66.64% 커버리지 |
| **Readable** | 90/100 | TypeScript 타입 안전성, 명확한 네이밍 |
| **Unified** | 95/100 | Prettier 포맷팅, 일관된 코드 스타일 |
| **Secured** | 85/100 | 입력 검증, 에러 핸들링 |
| **Trackable** | 90/100 | Git 히스토리, 문서화 |
...
본 프로젝트는 운영체제의 데드락 이론을 AI 멀티 에이전트 시스템에 성공적으로 적용했습니다.

**기술적 성과:**
- Wait-For Graph 기반 데드락 탐지 시스템 구현 (O(V+E) 시간 복잡도)
- 5가지 희생자 선택 전략 구현 및 합의 기반 선택 메커니즘
- 은행원 알고리즘 기반 데드락 회피 시스템
- 체크포인트/롤백 기반 에이전트 상태 복구
- REST API + WebSocket 기반 실시간 모니터링
...
1. **OS 이론의 AI 응용:** 검증된 운영체제 이론을 새로운 도메인에 적용
2. **실용적 해결책:** 실제 AI 서비스에서 발생하는 자원 경합 문제 해결 방안 제시
3. **정량적 분석:** 다양한 전략의 성능 비교 및 최적 전략 도출
