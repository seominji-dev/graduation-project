...
const envSchema = z.object({
  ...
  MONGODB_URI: z
    .string()
    .url()
    .default('mongodb://localhost:27017/checkpointing'),
  ...
  CHECKPOINT_INTERVAL_MS: z
    ...
    .default('30000')
    .transform((val) => parseInt(val, 10))
    .pipe(z.number().min(1000)), // Minimum 1 second
  MAX_CHECKPOINTS_PER_AGENT: z
    .string()
    .default('10')
    .transform((val) => parseInt(val, 10))
    .pipe(z.number().min(1)),
  MAX_STATE_SIZE_BYTES: z
    .string()
    .default('10485760') // 10MB
    .transform((val) => parseInt(val, 10))
    .pipe(z.number().min(1024)), // Minimum 1KB
...
export const config = {
  ...
  checkpointing: {
    ...
    maxCheckpointsPerAgent: env.MAX_CHECKPOINTS_PER_AGENT,
    maxStateSizeBytes: env.MAX_STATE_SIZE_BYTES,
  },
  ollama: {
