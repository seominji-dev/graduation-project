...
```bash
# 저장소 포크 후 클론
git clone https://github.com/YOUR_USERNAME/memory-manager.git
cd memory-manager

# 원격 저장소 추가
git remote add upstream https://github.com/ORIGINAL_OWNER/memory-manager.git
```
...
<개발-서버-실행>
```bash
# Redis, ChromaDB, MongoDB 시작
docker-compose up -d

# Ollama 시작 및 임베딩 모델 다운로드
...
=== 명명 규칙
<명명-규칙>
- 파일명: 카멜 케이스 (`hierarchicalMemoryManager.ts`)
- 인터페이스: `I` 접두사 없이 파스칼 케이스 (`MemoryPage`)
- 타입: 파스칼 케이스 (`CacheLevel`)
- 함수: 동사로 시작 (`getPage`, `evictLRU`)
...
```typescript
/**
 * Retrieve memory page from hierarchical cache
 * @param agentId - Agent identifier
 * @param key - Memory key
 * @returns Memory page or null if not found
 */
async getPage(agentId: string, key: string): Promise<MemoryPage | null> {
  // L1 캐시에서 먼저 조회
  // L1 미스 시 L2 검색 후 L1에 캐싱
  // L2 미스 시 L3 검색 후 L1, L2에 캐싱
  ...
}
```
...
<예시>
```bash
git commit -m "feat(cache): add LFU eviction policy"
git commit -m "fix(lru): resolve cache statistics race condition"
