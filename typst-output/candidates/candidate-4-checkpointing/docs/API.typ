...
#strong[기본 URL]: `http://localhost:3002/api`
...
state | object | Y | 에이전트 상태 (JSON 직렬화 가능) | | state.messages
| array | N | 메시지 히스토리 | | state.variables | object | N | 변수
상태 | | state.executionPosition | object | N | 실행 위치 정보 | |
state.status | string | N | 에이전트 상태 (running, paused, etc.) | |
options.type | string | N | 체크포인트 유형 (full, incremental) | |
options.description | string | N | 체크포인트 설명 | | options.tags |
array | N | 태그 목록 | | options.ttl | number | N | 생존 시간 (초),
기본값: 무제한 |
...
fallbackToLatest | boolean | N | 실패 시 최신 체크포인트 시도 (기본값:
true) |
...
<증분-체크포인트-incremental-checkpointing>
=== 6.1 증분 체크포인트 생성
<증분-체크포인트-생성>
...
#figure(
  align(center)[#table(
    ...
    [CHECKPOINT\_NOT\_FOUND], [체크포인트를 찾을 수 없음],
    [CHECKPOINT\_CORRUPTED], [체크포인트 데이터 손상],
    [STATE\_TOO\_LARGE], [상태 크기가 최대치 초과],
    [INTEGRITY\_CHECK\_FAILED], [무결성 검증 실패],
    [SERIALIZATION\_ERROR], [직렬화 오류],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    columns: 3,
    align: (auto,auto,auto,),
    table.header([체크포인트 유형], [평균 크기], [압축 효과],),
    table.hline(),
    [전체 (Full)], [1-5 MB], [없음],
...
=== 10.1 체크포인트 타이밍
<체크포인트-타이밍>
#strong[적절한 체크포인트 지점]: - 중요한 작업 완료 후 - 사용자 입력
처리 후 - 장기 계산 시작 전 - 상태 변경이 있을 때
...
#figure(
  align(center)[#table(
    ...
    [Incremental Backup], [Incremental Checkpoint], [변경된 부분만
    저장],
