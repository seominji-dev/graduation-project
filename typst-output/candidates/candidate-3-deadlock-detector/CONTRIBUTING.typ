...
```bash
# 저장소 포크 후 클론
git clone https://github.com/YOUR_USERNAME/deadlock-detector.git
cd deadlock-detector

# 원격 저장소 추가
git remote add upstream https://github.com/ORIGINAL_OWNER/deadlock-detector.git
```
...
=== 명명 규칙
<명명-규칙>
- 파일명: 카멜 케이스 (`cycleDetector.ts`)
- 인터페이스: `I` 접두사 없이 파스칼 케이스 (`WaitForEdge`)
- 타입: 파스칼 케이스 (`VictimSelectionStrategy`)
- 함수: 동사로 시작 (`detectCycle`, `selectVictim`)
 ...
 * @param graph - The wait-for graph to analyze
 * @returns Array of detected cycles
 */
function detectCycle(graph: WaitForGraph): Cycle[] {
  // DFS를 사용한 사이클 탐지
  // 백 엣지 발견 시 사이클로 간주
  // 시간 복잡도: O(V + E)
  ...
...
<예시>
```bash
git commit -m "feat(detector): add Tarjan's SCC algorithm"
git commit -m "fix(recovery): resolve rollback state inconsistency"
git commit -m "docs: update WFG visualization guide"
```
