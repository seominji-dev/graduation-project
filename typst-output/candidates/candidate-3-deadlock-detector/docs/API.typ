...
탐지와 다양한 희생자 선택 전략을 제공합니다.

#strong[기본 URL]: `http://localhost:3003/api`
...
priority | number | N | 우선순위 (1-10, 기본값: 5) | | metadata | object
| N | 추가 메타데이터 |
...
#strong[필드 설명]: | 필드 | 타입 | 필수 | 설명 |
|------|------|------|------| | name | string | Y | 자원 이름 | | type |
string | N | 자원 유형 (computational, storage, network) | | instances |
number | N | 인스턴스 수 (기본값: 1) | | metadata | object | N | 추가
메타데이터 |
...
<데드락-감지-및-회복-deadlock-detection-recovery>
=== 4.1 데드락 감지
<데드락-감지>
...
#strong[지원하는 전략]: - `lowest_priority`: 가장 낮은 우선순위 에이전트
선택 - `youngest`: 가장 최근에 생성된 에이전트 선택 - `most_resources`:
가장 많은 자원을 보유한 에이전트 선택 - `least_dependencies`: 가장 적은
의존성을 가진 에이전트 선택
...
#figure(
  align(center)[#table(
    ...
    [RESOURCE\_NOT\_FOUND], [자원을 찾을 수 없음],
    [RESOURCE\_ALREADY\_ALLOCATED], [자원이 이미 할당됨],
    [DEADLOCK\_DETECTED], [데드락이 감지됨],
    [UNSAFE\_STATE], [안전하지 않은 상태],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    columns: (42.86%, 28.57%, 28.57%),
    align: (auto,auto,auto,),
    table.header([OS 개념], [구현], [설명],),
    table.hline(),
    [Wait-For Graph], [WaitForGraph], [에이전트와 자원 간의 대기 관계를
    그래프로 표현],
    [Cycle Detection], [DFS (Depth-First Search)], [그래프에서 사이클
    탐지],
    [Victim Selection], [다양한 전략], [데드락 해결을 위한 희생자 선택],
    [Banker's Algorithm], [Safety Checker], [데드락 회피를 위한 안전
    상태 검사],
    [Checkpointing], [상태 저장 및 복구], [롤백을 위한 에이전트 상태
    저장],
  )]
  , kind: table
  )
