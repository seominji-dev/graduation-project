...
#figure(
  align(center)[#table(
    ...
    table.header([구성 요소], [최소 버전], [권장 버전], [비고],),
    table.hline(),
...
#strong[자원 유형:] | 유형 | 설명 | 예시 | |------|------|------| |
`computational` | 연산 자원 | GPU, CPU | | `storage` | 저장 자원 |
디스크, DB 연결 | | `network` | 네트워크 자원 | API 슬롯, 소켓 | |
`memory` | 메모리 자원 | RAM, 캐시 | | `custom` | 사용자 정의 | 기타 |
...
#strong[전략 옵션:] | 전략 | 설명 | |------|------| | `lowest_priority`
| 가장 낮은 우선순위 | | `youngest` | 가장 최근 생성 | |
`most_resources` | 가장 많은 자원 보유 | | `least_dependencies` | 가장
적은 의존성 | | `random` | 무작위 선택 |
...
==== 문제: 데드락이 감지되지 않음
<문제-데드락이-감지되지-않음>
#strong[가능한 원인:] 1. 그래프에 사이클이 없음 2. 탐지 간격이 너무 김
\3. 엣지가 올바르게 추가되지 않음
