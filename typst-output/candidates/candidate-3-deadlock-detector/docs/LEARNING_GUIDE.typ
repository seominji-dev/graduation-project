...
운영체제를 학습하는 학생, AI 시스템 개발자

#line(length: 100%)

== 목차
<목차>
+ #link(<1-os-데드락-개념-설명>)[OS 데드락 개념 설명]
+ #link(<2-멀티-에이전트-시스템-적용>)[멀티 에이전트 시스템 적용]
+ #link(<3-실습-예제>)[실습 예제]
+ #link(<4-연습-문제>)[연습 문제]
...
=== 1.1 데드락이란?
<데드락이란>
#strong[데드락(Deadlock, 교착 상태)]은 두 개 이상의 프로세스가 서로가
가진 자원을 기다리며 영원히 대기하는 상태입니다.
...
=== 1.2 데드락의 4가지 필요조건
<데드락의-4가지-필요조건>
데드락이 발생하려면 다음 #strong[네 가지 조건이 동시에] 만족되어야
합니다.
...
#strong[핵심 원리:] - 노드 = 프로세스 - 엣지 = 대기 관계 (A→B: A가 B의
자원을 기다림) - #strong[사이클 = 데드락]
...
#figure(
  align(center)[#table(
    ...
    table.header([방법], [설명], [장점], [단점],),
    table.hline(),
    [#strong[예방]], [4가지 조건 중 하나 제거], [데드락 원천
    차단], [자원 낭비],
    [#strong[회피]], [안전 상태만 유지], [높은 활용도], [복잡함],
    [#strong[탐지]], [발생 후 감지/해결], [최대 활용도], [복구 비용],
    [#strong[무시]], [데드락 무시], [단순함], [시스템 장애],
  )]
  , kind: table
  )
...
=== 2.1 AI 에이전트 시스템이란?
<ai-에이전트-시스템이란>
AI 에이전트는 특정 목표를 달성하기 위해 자율적으로 행동하는 소프트웨어
프로그램입니다.
...
#figure(
  align(center)[#table(
    ...
    table.header([OS 개념], [AI 에이전트 시스템], [예시],),
    table.hline(),
...
#strong[질문:] 1. 상호 배제 조건은 무엇인가요? 2. 점유 대기 조건은
무엇인가요? 3. 비선점 조건은 무엇인가요? 4. 순환 대기 조건은 무엇인가요?

정답 보기
+ #strong[상호 배제:] DB 연결은 한 에이전트만 사용 가능
+ #strong[점유 대기:] Agent-A가 DB1을 보유하면서 DB2를 요청
+ #strong[비선점:] 에이전트가 명시적으로 해제해야 함
+ #strong[순환 대기:] A→B→A 순환 구조
...
#strong[질문:] 1. 그래프를 그리세요. 2. 사이클이 있나요? 있다면 경로는?
\3. 데드락이 발생했나요?
...
+ 사이클 존재: Agent-1 → Agent-2 → Agent-3 → Agent-1
...
#strong[질문:] 1. Lowest Priority 전략의 희생자는? 2. Youngest 전략의
희생자는? 3. Most Resources 전략의 희생자는?

정답 보기
+ #strong[Lowest Priority:] Agent-B (priority=2)
+ #strong[Youngest:] Agent-B (30분, 가장 최근)
+ #strong[Most Resources:] Agent-A (3개)
...
#strong[질문:] 1. 각 에이전트의 Need(필요량)를 계산하세요. 2. 안전
순서를 구하세요. 3. 이 상태는 안전한가요?
...
#strong[질문:] 1. 외부 for 루프의 복잡도는? 2. dfsVisit의 복잡도는? 3.
전체 알고리즘의 시간 복잡도는?

정답 보기
+ 외부 루프: O(V) - 모든 노드 순회
+ dfsVisit: O(E/V) 평균 - 각 노드의 이웃 탐색
+ 전체: #strong[O(V + E)] - 각 노드와 엣지를 최대 한 번씩 방문
...
=== 추가 학습
<추가-학습>
- #link("https://ocw.mit.edu/courses/6-828-operating-system-engineering-fall-2012/")[MIT OCW - Operating Systems]
- #link("https://www.cs.uic.edu/~jbell/CourseNotes/OperatingSystems/7_Deadlocks.html")[Banker's Algorithm Visualization]
