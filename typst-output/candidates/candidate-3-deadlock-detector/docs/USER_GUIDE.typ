...
== 시작하기
<시작하기>
이 가이드는 Deadlock Detector 시스템을 설치, 구성, 사용하는 방법을
안내합니다. Deadlock Detector는 운영체제의 데드락 감지 및 회복
알고리즘을 AI/LLM 다중 에이전트 시스템에 적용한 시스템입니다.
...
=== 4.2 데드락 해결 프로세스
<데드락-해결-프로세스>
+ #strong[감지]: Wait-For Graph에서 사이클 탐지
+ #strong[분석]: 사이클에 포함된 에이전트와 자원 식별
+ #strong[선택]: 희생자 선택 전략 적용
+ #strong[체크포인트]: 희생자 상태 저장
+ #strong[롤백]: 희생자 종료 및 자원 해제
+ #strong[복구]: 저장된 상태에서 에이전트 재시작
...
#strong[원인]: 실제로는 데드락이 아닌데 감지됨

#strong[해결책]: - Wait-For Graph 타임아웃 조정 - 더 정확한 사이클 탐지
알고리즘 사용 - 자원 예약 시간 확인
