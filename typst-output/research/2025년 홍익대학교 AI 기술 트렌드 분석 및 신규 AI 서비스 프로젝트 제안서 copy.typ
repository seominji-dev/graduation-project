...
<하이브리드-rag-검색-엔진-dense-sparse-re-ranking>
==== 문제 정의
<문제-정의-1>
...
RRF(Reciprocal Rank Fusion)로 결과 통합 - Cross-encoder Re-ranking으로
최종 순위 결정
...
- 조항 간 참조 관계(예: "제3조의 규정에 따라…")가 있어 단순 검색으로
  불충분

==== 해결 방안
<해결-방안-2>
법령/규정 특화 RAG 시스템 구축 - 조항 단위 청킹 + 참조 관계 그래프 구축
...
유사도 임계값 조정으로 캐시 히트율/정확도 트레이드오프 관리 - OS의
페이지 교체 알고리즘(LRU, LFU)을 의미론적 캐싱에 적용
...
==== 학술적 가치
<학술적-가치-3>
- #strong[OS 캐시 이론]을 의미론적 도메인에 확장
- 유사도 임계값별 성능 비교 실험
...
==== 문제 정의
<문제-정의-4>
- 정적 분석 도구(ESLint, Semgrep)는 취약점을 발견하지만 왜 위험한지 설명
  부족
- 주니어 개발자가 결과를 이해하고 수정하기 어려움

==== 해결 방안
<해결-방안-4>
정적 분석 결과 + 코드 컨텍스트를 LLM에 전달 - 취약점 원인, 공격
시나리오, 수정 방법을 자연어로 설명 - OWASP Top 10 등 보안 지식 기반 RAG
연동
...
<분산-llm-추론-게이트웨이-multi-provider-load-balancing>
==== 문제 정의
<문제-정의-5>
...
==== 해결 방안
<해결-방안-5>
OpenAI, Claude, Gemini 등 여러 제공자를 풀로 관리 - 비용/속도/품질 기반
동적 라우팅 - 헬스체크 + Fallback 로직
...
==== 학술적 가치
<학술적-가치-5>
- #strong[OS 프로세스 스케줄링/로드밸런싱]을 AI 서빙에 적용
...
Claim Extraction → Evidence Retrieval → Verification - 신뢰도 점수 산출
...
==== 해결 방안
<해결-방안-7>
시스템 로그를 RAG로 인덱싱 + LLM이 근본 원인 분석(RCA) - 이상 패턴 탐지
\+ 유사 장애 사례 검색 - 단계별 진단 가이드 제공
...
==== 학술적 가치
<학술적-가치-7>
- #strong[AIOps(AI for IT Operations)] 분야 연구
- 시스템 관리 자동화
...
<로컬-프라이버시-ai-에이전트-on-device-llm>
==== 문제 정의
<문제-정의-8>
...
==== 해결 방안
<해결-방안-8>
경량 LLM(Phi-3, Gemma 2B)을 로컬에서 구동 - 민감도 분류 → 민감 데이터는
로컬, 비민감은 클라우드 - 하이브리드 파이프라인
...
\- OpenAPI 스펙 자동 생성 - 코드 변경 시 문서 자동 업데이트
...
#figure(
  align(center)[#table(
    ...
    명확한 OS 연관성],
    [�], [#strong[LLM 응답 시맨틱 캐싱 (\#4)]], [OS 페이지 교체
    알고리즘(LRU, LFU)을 의미론적 캐싱에 확장. 캐시 히트율 실험 가능],
    [🥉], [#strong[시스템 장애 로그 분석 AI (\#8)]], [시스템 로그/커널
    이벤트 분석. AIOps라는 실용적 분야 + OS 지식 필수],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    ...
    table.header([순위], [프로젝트], [추천 이유],),
    table.hline(),
    [🥇], [#strong[LLM 환각 탐지 시스템 (\#7)]], [AI 신뢰성 연구의 핵심
    주제. 논문 작성 가장 용이],
    [🥈], [#strong[하이브리드 RAG 검색 엔진 (\#2)]], [검색 기법 비교
    실험으로 정량적 성능 평가 가능],
    [🥉], [#strong[한국어 규정/법령 RAG (\#3)]], [도메인 특화 + 한국어
    NLP + Knowledge Graph 융합],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    ...
    table.header([순위], [프로젝트], [추천 이유],),
    table.hline(),
    [🥇], [#strong[분산 LLM 추론 게이트웨이 (\#6)]], [기업에서 바로 사용
    가능. 비용 절감 + 안정성 향상],
    [🥈], [#strong[API 문서 자동 생성 (\#10)]], [개발자 생산성 도구.
    오픈소스로 배포 시 포트폴리오 효과],
    [🥉], [#strong[한국어 규정/법령 RAG (\#3)]], [학교 학칙, 정부 규정
    등 실제 사용자에게 직접적 가치 제공],
  )]
  , kind: table
  )
...
#emph[작성일: 2026년 1월 1일] #emph[최종 수정: 2026년 1월 1일 - 학술적
깊이 있는 심화 프로젝트 10개로 전면 개편]
