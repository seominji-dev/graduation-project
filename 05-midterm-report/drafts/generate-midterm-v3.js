        ...
        children: [
          ...
          multiRunParagraph([
            ...
            { text: '는 여러 개의 대기열을 두고, 작업의 특성에 따라 우선순위를 자동으로 조정하는 알고리즘이다. ' +
              '새 작업은 가장 높은 우선순위 큐에 들어가고, 시간 할당량 안에 끝나지 않으면 한 단계 낮은 큐로 내려간다. ' +
              '짧은 작업은 빠르게, 긴 작업은 천천히 처리되는 효과가 있다 [3].' }
          ]),
          multiRunParagraph([
            { text: 'WFQ(Weighted Fair Queuing)', bold: true },
            { text: '는 네트워크 분야에서 사용되는 공정한 자원 배분 알고리즘이다. 각 사용자에 가중치를 ' +
              '부여하고, 가중치가 클수록 더 많은 자원을 받도록 설계되어 있다. 구체적인 동작 방식은 4.3절에서 설명한다.' }
          ]),

          // 2.2 LLM 서빙 시스템
          heading2('2.2 LLM 서빙 시스템'),
          textParagraph(
            'LLM 서빙이란 학습이 완료된 LLM을 사용자에게 제공하는 것을 뜻한다. 대표적인 오픈소스 도구로 ' +
            'vLLM [4]과 Hugging Face TGI [5]가 있다. vLLM은 PagedAttention 기반의 높은 처리 성능을, TGI는 ' +
            '배치 처리(Continuous Batching) 기능을 제공하지만, 두 도구 모두 다중 사용자 환경에서의 요청 우선순위 ' +
            '관리나 공정성 보장 기능은 공식 문서에서 확인하지 못하였다. 본 프로젝트는 이 부분에 스케줄링 이론을 ' +
            '적용해보려고 한다.'
          ),
          ...
          textParagraph(
            '다만 MLFQ의 경우, 비선점형으로 운영하면 요청이 중단 없이 끝까지 처리되어 큐 간 이동이 발생하지 않으므로, ' +
            '짧은 요청을 우선 처리하는 핵심 효과가 나타나지 않았다 (5.2절 참조). 이것이 MLFQ 알고리즘 자체의 한계인지, ' +
            'LLM 환경의 제약 때문인지 확인하기 위해 선점형 모드를 시뮬레이션 환경에서 추가 구현하였다 (5.3절).'
          ),
          ...
          bulletItemBold(
            'Priority Scheduling: ',
            '요청 우선순위(URGENT, HIGH, NORMAL, LOW)에 따라 긴급한 요청을 먼저 처리한다. 오래 기다린 요청의 ' +
            '우선순위를 높여주는 에이징(Aging) 기법도 구현하였다(5초 주기).'
          ),
          bulletItemBold(
            'MLFQ: ',
            '4단계 큐(Q0~Q3)를 두고, 큐별로 다른 시간 할당량을 적용한다. OSTEP [3]의 표준 MLFQ 규칙을 따르며, ' +
            '기아 방지를 위해 5초마다 모든 요청을 최상위 큐로 올려준다.'
          ),
          bulletItemBold(
            'WFQ: ',
            '사용자 등급(Enterprise~Free)에 따라 가중치(100, 50, 10, 1)를 부여하고, 가중치를 반영한 가상 완료 시각을 ' +
            '계산하여 서비스를 차등 제공한다.'
          ),
          ...
          textParagraph(
            ...
            '핵심은 schedulers 모듈로, BaseScheduler를 상속받는 4개 스케줄러(FCFS, Priority, MLFQ, WFQ)가 ' +
            '각각 enqueue()와 dequeue() 메서드를 구현한다. 같은 인터페이스를 사용하므로, 서버 코드를 수정하지 않고 ' +
            '환경변수만으로 알고리즘을 전환할 수 있다.'
          ),
          ...
          textParagraph(
            ...
            'MLFQ는 OSTEP [3]의 5가지 규칙(표 4)에 따르며, 큐별 시간 할당량은 표 5와 같다.'
          ),
          emptyLine(),
          ...
          multiRunParagraph([
            { text: 'Priority Scheduling: ', bold: true },
            { text: '우선순위에 따른 대기시간 차이가 뚜렷하게 나타났다. URGENT 요청은 평균 약 42ms로 거의 즉시 처리된 ' +
              '반면, LOW 요청은 약 4,600ms로 크게 늘어났다. 전체 평균이 FCFS보다 약간 높은 것은, 높은 우선순위를 먼저 ' +
              '처리한 만큼 낮은 우선순위의 대기가 길어지기 때문이다. 에이징 효과는 요청당 처리 시간이 짧아(10~100ms) ' +
              '이번 실험에서는 관측되지 않았으며, 실서버 실험에서 검증할 계획이다 (6절 참조).' }
          ]),
          multiRunParagraph([
            { text: 'WFQ: ', bold: true },
            { text: 'Enterprise 등급(가중치 100)의 평균 대기시간은 약 430ms, Free 등급(가중치 1)은 약 4,540ms로 ' +
              '약 10배 차이가 나타나, 가중치에 비례한 차등 서비스를 확인하였다. JFI는 0.32로 낮게 나왔는데, 이는 ' +
              '가중치가 다른 사용자에게 동일 건수를 배정한 실험 설계의 영향이다. 같은 등급 내에서는 대기시간 편차가 ' +
              '작아 등급 내 배분은 고르게 이루어졌다.' }
          ]),
          multiRunParagraph([
            { text: 'MLFQ: ', bold: true },
            { text: '비선점형에서는 FCFS와 동일한 결과가 나왔다 (평균 대기시간 2,572ms, 처리량 18.0 req/s). ' +
              '모든 요청이 중단 없이 완료되어 큐 간 이동이 발생하지 않기 때문이며, 이 결과가 3.3절의 선점형 ' +
              '시뮬레이션을 추가 구현한 계기이다.' }
          ]),
          ...
          textParagraph(
            'MLFQ의 이론적 효과를 검증하기 위해, 선점형 시뮬레이션을 추가로 수행하였다 (표 6의 추가 실험 조건). ' +
            '요청 수 500건, 처리 시간 최대 10초, 버스트 도착 구조로 기본 실험보다 규모를 크게 늘렸으며, 5개 시드로 ' +
            '반복하였다. 선점형에서는 요청의 중단과 재개가 반복되므로, 지표를 응답시간(도착~완료)으로 측정하였다.'
          ),

          // 표 9: MLFQ 선점형 vs FCFS (요청 유형별)
          tableCaption('표 8. MLFQ 선점형 vs FCFS 비교 (요청 유형별)'),
          new Table({
