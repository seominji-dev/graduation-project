...
const AgentSchema = new Schema({
  ...
  state: { type: String, enum: ['active', 'waiting', 'blocked', 'terminated'], default: 'active' },
...
const ResourceSchema = new Schema({
  name: { type: String, required: true },
  type: { type: String, enum: ['computational', 'storage', 'network', 'memory', 'custom'], default: 'custom' },
...
const DeadlockEventSchema = new Schema({
  ...
  resolutionAction: { type: String, enum: ['terminate', 'rollback', 'preempt'], default: null },
...
const RecoveryActionSchema = new Schema({
  ...
  actionType: { type: String, enum: ['terminate', 'rollback', 'preempt'], required: true },
