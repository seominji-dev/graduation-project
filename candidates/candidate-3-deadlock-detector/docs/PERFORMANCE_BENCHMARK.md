...
**버전:** 1.0.0
**작성일:** 2026-01-25
**테스트 환경:** Node.js 20 LTS, Apple M1 Pro, 16GB RAM

---

## 목차

1. [탐지 성능 분석](#1-탐지-성능-분석)
2. [희생자 선택 전략 비교](#2-희생자-선택-전략-비교)
3. [롤백 시간 분석](#3-롤백-시간-분석)
4. [대규모 그래프 성능](#4-대규모-그래프-성능)
...
| 사이클 크기 | Lowest Priority | Youngest | Most Resources | Composite |
|-------------|-----------------|----------|----------------|-----------|
...
| 에이전트 수 | 자원 유형 수 | 안전 검사 시간 | 요청 처리 시간 |
|-------------|-------------|---------------|---------------|
| 10 | 5 | < 1ms | < 1ms |
| 50 | 10 | 5ms | 6ms |
| 100 | 20 | 25ms | 28ms |
...
| 동시 요청 수 | 평균 응답 시간 | P95 응답 시간 | P99 응답 시간 |
|-------------|---------------|---------------|---------------|
| 10 | 5ms | 8ms | 12ms |
| 50 | 12ms | 25ms | 40ms |
| 100 | 25ms | 50ms | 80ms |
...
1. **병렬 DFS:** 다중 스레드 사이클 탐지
2. **증분 탐지:** 변경된 부분만 재탐지
3. **그래프 분할:** 대규모 그래프 분할 처리
4. **캐싱:** 중간 결과 캐싱
5. **네이티브 모듈:** 성능 크리티컬 부분 C++ 구현
