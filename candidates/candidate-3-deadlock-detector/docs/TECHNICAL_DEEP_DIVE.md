...
**버전:** 1.0.0
**작성일:** 2026-01-25
**대상 독자:** 시스템 아키텍트, 백엔드 개발자, OS 연구자

---

## 목차

1. [데드락 개념 및 조건](#1-데드락-개념-및-조건)
2. [DFS 기반 사이클 탐지 알고리즘](#2-dfs-기반-사이클-탐지-알고리즘)
3. [희생자 선택 전략 심층 비교](#3-희생자-선택-전략-심층-비교)
4. [롤백 메커니즘](#4-롤백-메커니즘)
5. [예방 vs 탐지 비교](#5-예방-vs-탐지-비교)
...
데드락(Deadlock)은 두 개 이상의 프로세스(또는 에이전트)가 서로가 가진 자원을 기다리며 무한히 대기하는 상태입니다. 이 상태에서는 외부 개입 없이는 어떤 프로세스도 진행할 수 없습니다.
...
| 전략 | 설명 | 장점 | 단점 | 본 시스템 적용 |
|------|------|------|------|---------------|
| 예방 (Prevention) | 4가지 조건 중 하나 제거 | 데드락 완전 방지 | 자원 활용도 저하 | 부분 적용 |
| 회피 (Avoidance) | 안전 상태 유지 | 높은 자원 활용도 | 오버헤드 | 은행원 알고리즘 |
| 탐지 (Detection) | 데드락 발생 후 감지 | 최대 자원 활용 | 복구 비용 | WFG + DFS |
| 무시 (Ignorance) | 데드락 무시 | 단순함 | 시스템 장애 | 미적용 |
...
Wait-For Graph에서 사이클을 탐지하기 위해 깊이 우선 탐색(DFS)을 사용합니다. 그래프 이론에서 방향 그래프의 사이클 존재 여부는 DFS 중 **백 엣지(Back Edge)**의 발견으로 판단할 수 있습니다.
...
| 측면 | 복잡도 | 설명 |
|------|--------|------|
| **시간 복잡도** | O(V + E) | 각 노드와 엣지를 최대 한 번씩 방문 |
| **공간 복잡도** | O(V) | 색상 배열 + 재귀 스택 |
| **사이클 추출** | O(V) | 경로에서 사이클 부분 추출 |
| **전체** | O(V + E) | 선형 시간 알고리즘 |
...
복잡한 그래프에서 모든 강연결요소(SCC)를 찾아 사이클을 식별하는 Tarjan 알고리즘도 구현되어 있습니다:
...
4가지 Coffman 조건 중 하나를 제거하여 데드락 자체가 발생하지 않도록 합니다.
...
|------|------|------|------|
| **자원 활용도** | 낮음 | 중간 | 높음 |
| **오버헤드** | 낮음 | O(n^2 * m) | O(V + E) |
| **구현 복잡도** | 낮음 | 높음 | 중간 |
| **복구 비용** | 없음 | 없음 | 롤백 필요 |
| **실시간성** | 좋음 | 좋음 | 탐지 지연 |
| **적용 환경** | 정적 시스템 | 알려진 최대 요구량 | 동적 시스템 |
...
**장점:**
1. 은행원 알고리즘으로 대부분의 데드락 사전 차단
2. 예외적 상황은 탐지 + 회복으로 처리
3. 자원 활용도와 안전성의 균형
...
| 그래프 크기 | DFS 탐지 시간 | 은행원 검사 시간 | 롤백 시간 |
|------------|--------------|-----------------|----------|
| 10 노드 | < 1ms | < 1ms | ~5ms |
