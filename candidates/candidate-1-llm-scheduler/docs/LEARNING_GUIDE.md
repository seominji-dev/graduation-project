...
1. [서론](#1-서론)
2. [OS 스케줄링 기초](#2-os-스케줄링-기초)
3. [LLM 스케줄링 적용](#3-llm-스케줄링-적용)
4. [알고리즘별 학습](#4-알고리즘별-학습)
5. [실습 예제](#5-실습-예제)
6. [연습 문제](#6-연습-문제)
...
**스케줄링(Scheduling)**은 제한된 자원(CPU)을 여러 프로세스가 공유할 때, 어떤 프로세스에게 자원을 할당할지 결정하는 것입니다.
...
1. **CPU 이용률 (CPU Utilization)**: CPU가 얼마나 바쁘게 동작하는가
2. **처리량 (Throughput)**: 단위 시간당 완료되는 프로세스 수
3. **총처리 시간 (Turnaround Time)**: 요청 제출부터 완료까지 걸린 시간
4. **대기 시간 (Waiting Time)**: Ready Queue에서 대기한 시간
5. **응답 시간 (Response Time)**: 요청 제출부터 첫 응답까지 시간
...
**비선점형 (Non-preemptive)**
- 프로세스가 CPU를 자발적으로 반납할 때까지 계속 실행
- 구현이 단순
- 예: FCFS

**선점형 (Preemptive)**
- 스케줄러가 강제로 CPU를 회수할 수 있음
- 응답성이 좋음
- 예: Round Robin, MLFQ (부분적)
...
| 프로세스 상태 | 요청 상태 (Pending, Queued, Processing, Completed) |
| Time Quantum | 요청 처리 시간 제한 |
| Context Switch | 요청 전환 (한 요청 완료 후 다음 요청 처리) |

### 3.2 왜 LLM에 스케줄링이 필요한가?

1. **비용 관리**: API 호출 비용이 높음
2. **자원 제한**: 동시 요청 수 제한 (Rate Limiting)
3. **응답성**: 급한 요청은 빨리 처리해야 함
4. **공정성**: 모든 사용자에게 공평한 서비스
...
**문제**: 낮은 우선순위 요청이 영원히 처리되지 않을 수 있음
...
1. 여러 개의 우선순위 큐 사용
2. 새 요청은 최고 우선순위 큐에서 시작
3. 시간을 많이 사용하면 낮은 큐로 강등
4. 주기적으로 모든 요청을 최고 큐로 부스팅
...
1. 각 테넌트에게 가중치 부여
2. 가상 시간(Virtual Time)으로 공정성 추적
3. 가상 종료 시간이 가장 작은 요청 먼저 처리
...
**문제 1**: 다음 요청들의 FCFS 평균 대기 시간을 계산하세요.
...
대기 시간:
- A: 0ms (도착 즉시 시작)
- B: 10ms - 2ms = 8ms (A 완료까지 대기)
- C: 15ms - 4ms = 11ms (A, B 완료까지 대기)

평균 대기 시간: (0 + 8 + 11) / 3 = 6.33ms
</details>

**문제 2**: 동일한 요청들에 대해 SJF(Shortest Job First)를 적용하면 평균 대기 시간은?

<details>
<summary>정답 보기</summary>

처리 순서: A -> B -> C (A가 먼저 도착해서 처리, B(5ms)가 C(8ms)보다 짧음)
...
**문제 3**: MLFQ에서 다음 시나리오의 최종 큐 레벨을 예측하세요.
...
요청 Y:
- Q0에서 시작, 1000ms 처리 후 타임 초과 -> Q1로 강등
- Q1에서 3000ms 처리 완료 (누적 4000ms)
- 최종 레벨: Q1
</details>

**문제 4**: WFQ에서 가상 종료 시간을 계산하세요.
...
테넌트 A의 가상 종료 시간:
F_A = max(50, 45) + (100 / 50) = 50 + 2 = 52

테넌트 B의 가상 종료 시간:
F_B = max(50, 60) + (100 / 10) = 60 + 10 = 70

결론: 테넌트 A의 요청이 먼저 처리됨 (F_A=52 < F_B=70)
</details>

### 6.3 심화 문제

**문제 5**: 다음 시나리오에서 각 알고리즘별 성능을 비교하세요.
...
**FCFS:**
- 장점: 구현 단순, 공정함
- 단점: 긴 요청이 짧은 요청을 블로킹 (Convoy Effect)
- 평균 대기 시간: 높음 (긴 요청 때문에)

**Priority:**
- 장점: 중요한 요청 우선 처리 가능
- 단점: 짧은/긴 구분 없이 우선순위만 봄
- 평균 대기 시간: 우선순위 분포에 따라 다름

**MLFQ:**
- 장점: 짧은 요청 빠르게 처리 (Q0에서 완료)
- 장점: 긴 요청도 결국 처리 (Q3에서)
- 장점: 자동 적응 (워크로드 예측 불필요)
- 단점: 구현 복잡도 높음
- 평균 대기 시간: 가장 낮음 (짧은 요청 최적화)

**결론**: 이 워크로드에는 MLFQ가 가장 적합
</details>
