...
describe('TenantRegistry', () => {
  ...
  describe('getTenantWeight', () => {
    ...
    it('should return default FREE tier weight for unknown tenant', () => {
      expect(registry.getTenantWeight('non-existent')).toBe(DEFAULT_WEIGHTS[TenantTier.FREE]);
    });
  ...
  describe('updateTenantTier', () => {
    it('should update tier for existing tenant', () => {
      ...
      const result = registry.updateTenantTier('tenant-1', TenantTier.ENTERPRISE);
    ...
    it('should update weight to default for new tier', () => {
      ...
      expect(registry.getTenantWeight('tenant-1')).toBe(DEFAULT_WEIGHTS[TenantTier.PREMIUM]);
    });

    it('should return false for non-existent tenant', () => {
      const result = registry.updateTenantTier('non-existent', TenantTier.ENTERPRISE);
      expect(result).toBe(false);
    });
    ...
    it('should handle all tier transitions', () => {
      ...
      for (const tier of tiers) {
        ...
        expect(registry.getTenantWeight('tenant-1')).toBe(DEFAULT_WEIGHTS[tier]);
      }
    });
  });
  ...
  describe('getAllTenants', () => {
    ...
    it('should return all registered tenants', () => {
      ...
      expect(tenants.map(t => t.id).sort()).toEqual(['tenant-1', 'tenant-2', 'tenant-3']);
    });
  ...
  describe('DEFAULT_WEIGHTS', () => {
    ...
    it('should have weight ratio of 100:50:10:1', () => {
      const ratio = {
        enterprise: DEFAULT_WEIGHTS[TenantTier.ENTERPRISE] / DEFAULT_WEIGHTS[TenantTier.FREE],
        premium: DEFAULT_WEIGHTS[TenantTier.PREMIUM] / DEFAULT_WEIGHTS[TenantTier.FREE],
        standard: DEFAULT_WEIGHTS[TenantTier.STANDARD] / DEFAULT_WEIGHTS[TenantTier.FREE],
      };
  ...
  describe('Integration Scenarios', () => {
    it('should handle typical multi-tenant workflow', () => {
      ...
      expect(registry.getTenant('tenant-standard')?.tier).toBe(TenantTier.PREMIUM);
    });
    ...
    it('should handle concurrent access simulation', () => {
      ...
      // Verify updates
      for (let i = 0; i < 50; i++) {
        expect(registry.getTenant(`tenant-${i}`)?.tier).toBe(TenantTier.PREMIUM);
      }
