...
export const apiRateLimiter = rateLimit({
  ...
  message: (_req: Request, res: Response): RateLimitErrorResponse => {
    ...
    return {
      ...
      message: 'Too many requests from this IP, please try again after ' + Math.ceil(RATE_LIMIT_WINDOW_MS / 60000) + ' minutes',
      retryAfter,
    };
  },
...
export const strictRateLimiter = rateLimit({
  ...
  message: (_req: Request, res: Response): RateLimitErrorResponse => {
    ...
    return {
      error: 'TooManyRequests',
      message: 'Too many requests to this endpoint, please try again after ' + Math.ceil(RATE_LIMIT_WINDOW_MS / 60000) + ' minutes',
      retryAfter,
    };
  },
  keyGenerator: (req: Request): string => {
